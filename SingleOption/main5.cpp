...
void test_european(MarketParameters& paras, EuropeanOption& eop, long nM=10000)
{
	...
	eop.CalcMC(paras, nM);
	std::vector<double> rsMc = eop.GetResult();
	...
	std::cout<< "adj delta "<<rs[1]<<std::endl;
	std::cout<< "adj gamma "<<rs[2]<<std::endl;
...
{//setpara
	...
	std::cout << "pv(FD) " << (autoop.GetHitFlag() ? "hitted " : "not hit ") << "->" << rs[0] << std::endl;
	//std::cout << "pv(MC) " << (autoop.GetHitFlag() ? "hitted " : "not hit ") << "nMC= " << nM << "-> " << rs_mc[0] << std::endl;
	std::cout << "delta(FD) " << (autoop.GetHitFlag() ? "hitted " : "not hit ") << "->" << rs[1] << std::endl;

}

void test_autocall__fd_inst_swip_first_sim(MarketParameters& paras, AutocallOption& autoop)
{//setpara
	double spot = paras.get_spot();
	long nM = 5;
	autoop.Simulation(paras,nM);
	std::vector<double> rs = autoop.GetResult();
...
void test_calc2_mc_for_vanilla(long n)
{
	...
	//EurPut.Calc(para);
	//EurPut2.CalcMC(paras, n);
	////EurPutMC.Calc(para,n);
	//EurPutMC.Calc(paras, n);
	...
	//vector<double> rs_mc_paras_calc2_module= EurPutMC.GetResult();
	...
	//std::cout << "Eurput.CalcMC " << rs_fdm_paras_calc2[0] << "=%price " << rs_fdm_paras_calc2[0] / refprice * 100 << "%" << std::endl;
	////cout << "pv by mc numMC="  <<n << "  "  << rs3[0] << "=%price " << rs3[0] / spot * 100 << "%" << std::endl;
	//cout << "EurputMC.calc=" << n << "  " << rs_mc_paras_calc2_module[0] << "=%price " << rs_mc_paras_calc2_module[0] / refprice * 100 << "%" << std::endl;
...
MarketParameters set_file(const char* volcsv)
{
	ifstream infile(volcsv); // for example
	string line = "";
